...
frappe.ui.form.on("User for callHippo", {
  refresh: function (frm) {
    frm.add_custom_button(__("Call Logs CallHippo"), function () {
      frappe.set_route("List", "CallHippo Call Logs", {
        caller_email: frm.doc.email_id,
      });
    });
  },
  before_save: async function (frm) {
    let authentication_token = await frappe.db.get_single_value(
      "CallHippo Settings",
      "token"
    );
    if (
      typeof cur_frm.doc.user_id == "undefined" ||
      cur_frm.doc.user_id == ""
    ) {
      ...
      await fetch(
        "https://web.callhippo.com/v1/user/edit/" + cur_frm.doc.user_id,
        {
          method: "PUT",
          headers: {
            Accept: "application/json",
